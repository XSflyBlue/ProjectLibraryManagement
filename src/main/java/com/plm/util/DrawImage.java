...
public class DrawImage {
    ...
    private static DrawImage instance=null;//单例模式
    
    public static synchronized DrawImage getInstance(){
        if(instance==null){
               instance=new DrawImage();
        }
        return instance;
    }
	...
	public static String getImage(HttpServletResponse response) throws IOException {
        String createTypeFlag = "nl";//生成数字和字母组合的验证码图片
        DrawImage drawImage = getInstance();
        //1.在内存中创建一张图片
        BufferedImage bi = new BufferedImage(WIDTH, HEIGHT,BufferedImage.TYPE_INT_RGB);
        //2.得到图片
        Graphics g = bi.getGraphics();
        //3.设置图片的背影色
        drawImage.setBackGround(g);
        //4.设置图片的边框
        drawImage.setBorder(g);
        //5.在图片上画干扰线
        drawImage.drawRandomLine(g);
        //6.写在图片上随机数
        ...
        String random = drawImage.drawRandomNum((Graphics2D) g,createTypeFlag);//根据客户端传递的createTypeFlag标识生成验证码图片
        //7.设置响应头通知浏览器以图片的形式打开
