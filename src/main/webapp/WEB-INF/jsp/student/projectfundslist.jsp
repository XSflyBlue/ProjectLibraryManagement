...
<head>
    ...
    <title>项目预算-大学生创新创业训练项目库管理系统</title>
		...
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal"
						aria-hidden="true">&times;</button>
					<h4 class="modal-title" id="myModalLabel">新建项目预算</h4>
				</div>
				<div class="modal-body" style="height:400px;overflow:auto;">
					<form role="form">
						<div class="form-group">
						    <input type="hidden" id="pfundId" name="pfundId" value="">
							<label for="name">此处填写预算信息</label>
							<textarea id ="pfundcontent" class="form-control" rows="20"></textarea>
						</div>
					</form>
				</div>
				<div class="modal-footer">
        ...
        success:function(data){//返回json结果
        	if(data!=null){
				...
				$(data.list).each(function(index,item){
                    ...
                    if(item.uType==0){//负责人
                    	if(item.fStatus==3||item.fStatus==1){//审核未通过可修改
                    		project +='<button type="button" class="am-btn am-btn-default am-btn-xs" data-toggle="modal" data-target="#newfund" '
                    		+'onclick="editfunds('+item.pbId+')">编辑</button>';
                    	}else if(item.fStatus==0){//待添加
                    		project +='<button type="button" class="am-btn am-btn-default am-btn-xs" data-toggle="modal" data-target="#newfund" '
                    		+'onclick="pfundId('+item.pbId+')">添加</button>';
                    	}
                    }
...
function editfunds(id){
	...
	$.ajax({
        ...
        success:function(data){//返回json结果
        	if(data!=null){
        		pretext = data.fContent;
        		$('#pfundcontent').val(data.fContent);
			}
        }
    }); 
}
...
function addfunds(){
    ...
    var pfundcontent = $('#pfundcontent').val();
    if(pfundId==''||pfundcontent=='')
    {
        alert('信息不完整');
        return false;
    }
    if(pretext==pfundcontent){
        alert('未更改');
        return false;
    }
    //异步请求
	$.ajax({
        ...
        //数据格式是key/value
        data:'projectId='+pfundId+'&pfundcontent='+pfundcontent,
        success:function(data){//返回json结果
        	if(data!=null){
        		if(data==1){
            		alert('操作成功');
		...
		$.ajax({
	        ...
	        success:function(data){//返回json结果
	        	if(data!=null){
	        		...
	        		if(data.fOpinion==null||data.fOpinion==''){
	        			opinion = '暂无评论';
	        		}else{
