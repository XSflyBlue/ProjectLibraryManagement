...
<head>
    ...
    <title>网站帮助管理-大学生创新创业训练项目库管理系统</title>
...
<body data-type="index">
    ...
    <!-- 新增帮助（Modal） -->
	<div class="modal fade" id="addhelp" tabindex="-1" role="dialog"
		aria-labelledby="myModalLabel" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
				...
				<div id="" class="form-group">
						<table class="table table-bordered">
							...
							<tbody>
								<tr>
									<td><input type="text" id="addhelptitle" class="form-control" size="30"><input type="hidden" id="helptitle"></td>
								</tr>
							</tbody>
							<thead>
								<tr>
							...
							<tbody>
								<tr>
									<td><input type="text" id="addhelptype" class="form-control" size="30"><input type="hidden" id="helptype"></td>
								</tr>
							</tbody>
						</table>
						...
						<div>
							<h4>
								帮助内容：<span class="close" data-close="note"></span>
							</h4>
							<textarea id ="addhelpcontent" class="form-control" rows="5"></textarea>
							<input type="hidden" id="helpcontent">
						</div>
					</div>
				</div>
				<div class="modal-footer">
				    <button type="submit" class="btn btn-info" onclick="addhelp()">提交</button>
					<button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
				</div>
			</div>
			<!-- /.modal-content -->
				...
				$(data.list).each(function(index,item){
					...
					if(role==0){//修改、删除权限
						links += '<button type="button" class="am-btn am-btn-default am-btn-xs am-text-secondary" '
                            +' data-toggle="modal" data-target="#addhelp" '
                            +'onclick="puthelpid('+item.hId+')"> 编辑</button>'
                            +'<button type="button" class="am-btn am-btn-default am-btn-xs am-text-danger am-hide-sm-only" '
                            +'onclick="deletehelp('+item.hId+')"> 删除</button>';
					}
...
function deletehelp(helpid) {
	if(confirm("是否确认删除帮助？")){
		...
		$.ajax({
	        type:'post',
	        url:'${pageContext.request.contextPath}/deletehelp',
	        //数据格式是key/value
	        data:'helpid='+helpid,
	        success:function(data){//返回json结果
	        	if(data!=null){
...
//新增（编辑）帮助
function addhelp() {
	var helptitle = $('#addhelptitle').val();
	var helptype = $('#addhelptype').val();
	var helpcontent = $('#addhelpcontent').val();
	var helpid = $('#helpid').val();
	
	if(helptitle==''||helptype==''||helpcontent==''){
        alert('信息不完整');
        return false;
	}
	if(helpid==''||helpid==null){//新增
		if(confirm("是否确认新增帮助？")){
			//异步请求
			$.ajax({
		        type:'post',
		        url:'${pageContext.request.contextPath}/addhelp',
		        //数据格式是key/value
		        data:'helptitle='+helptitle+'&helpcontent='+helpcontent+'&helptype='+helptype,
		        success:function(data){//返回json结果
		        	if(data!=null){
		        		if(data==0){
		        			alert('操作失败');
		...
		var originalhelptitle = $('#helptitle').val();
	    var originalhelptype = $('#helptype').val();
	    var originalhelpcontent = $('#helpcontent').val();

	    if(originalhelptitle==helptitle && originalhelptype==helptype && originalhelpcontent==helpcontent){
	        alert('信息未更改');
	        return false;
	    }
		if(confirm("是否确认修改帮助？")){
			...
			$.ajax({
		        ...
		        url:'${pageContext.request.contextPath}/addhelp',
		        //数据格式是key/value
		        data:'helpid='+helpid+'&helptitle='+helptitle+'&helpcontent='+helpcontent+'&helptype='+helptype,
		        success:function(data){//返回json结果
		        	if(data!=null){
		        		if(data==0){
		        			alert('操作失败');
...
$(function(){
	//功能列表
	$('#j_helpmenu').removeClass("nav-link tpl-left-nav-link-list");
	$('#j_helpmenu').addClass("nav-link active");
	...
	if(role==0){
		...
		var menu = '<div class="am-u-sm-12 am-u-md-6"><div class="am-btn-toolbar">'
			...
			+'data-toggle="modal" data-target="#addhelp" onclick="puthelpid(0)">'
	...
	$('#addhelp').on('show.bs.modal', function () {
		...
		$.ajax({
	        ...
	        success:function(data){
				if(data!=null&&data!=''){
					$('#addhelptitle').val(data.hTitle);
					$('#helptitle').val(data.hTitle);
					$('#addhelptype').val(data.hType);
					$('#helptype').val(data.hType);
					$('#helpcontent').val(data.hContent);
					$('#addhelpcontent').val(data.hContent);
				}else{
					$('#addhelptitle').val('');
					$('#helptitle').val('');
					$('#addhelptype').val('');
