...
function edituserinfo() {
	...
	var newnote2 = $('#newnote2').val();
	var newcollege = $('#newcollege').val();
	var newmajor = $('#newmajor').val();
	if(uName==''||email==''||tel==''||note2==''||college==''||major==''){
        alert('信息不完整');
        return false;
	}
	
	if(uName==newuName && email==newemail && tel==newtel 
			&&note2==newnote2 && college==newcollege && major==newmajor){
        alert('信息未修改');
        return false;
	}
	...
	$.ajax({
        ...
        //数据格式是key/value
        data:'uName='+uName+'&email='+email+'&tel='+tel+'&jobTitle='+note2+'&major='+major+'&college='+college,
			          ...
			          <c:if test="${user!=null&&user.role==5}">
			           ...
			           <c:if test="${user!=null&&user.role==4||user.role==3}">
			            ...
			            <input type="text" class="form-control" id="note2" name="note2" 
			                value="${user.jobTitle}" placeholder="">
			                <input type="hidden" id="newnote2" name="newnote2" value="${user.jobTitle}">
			           </c:if>
			          </div>
			          <div class="col-md-6">
...
<div class="tpl-left-nav tpl-left-nav-hover">
	<div class="tpl-left-nav-title">
		<c:if test="${user.role eq '3'}">评审专家</c:if>
		<c:if test="${user.role eq '4'}">指导教师</c:if>
		功能列表
	</div>
	<div class="tpl-left-nav-list">
		<ul class="tpl-left-nav-menu">
			...
			<li class="tpl-left-nav-item"><a href="${pageContext.request.contextPath}/user/loginCheck"
			...
			<li class="tpl-left-nav-item"><a id="j_menu" href="javascript:;"
				...
				<ul id="j_submenu" class="tpl-left-nav-sub-menu">
					...
					</a> <a id="j_mid" href="${pageContext.request.contextPath}/projectinfo/midlist"> <i class="am-icon-angle-right"></i>
							<span>中期审核</span>
					</a> <a id="j_end" href="${pageContext.request.contextPath}/projectinfo/endlist"> <i class="am-icon-angle-right"></i>
							<span>验收审核</span>
					</a> <a id="j_week" href="${pageContext.request.contextPath}/projectinfo/userprojectlist"> <i class="am-icon-angle-right"></i>
							<span>周报审核</span>
