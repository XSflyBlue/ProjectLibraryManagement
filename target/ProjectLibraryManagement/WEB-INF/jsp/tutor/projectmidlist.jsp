...
<head>
    ...
    <title>中期审核-大学生创新创业训练项目库管理系统</title>
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-body" style="height:400px;overflow:auto;">
					...
					<input type="hidden" id="midpId" name="midtpId" value="">
		...
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal"
						aria-hidden="true">&times;</button>
					<h4 class="modal-title" id="myModalLabel">项目中期审核</h4>
				</div>
				<div class="modal-body" style="height:400px;overflow:auto;">
					<form role="form">
        ...
        complete:function()
        {
        	...
        	if(confirm("是否确认重置审核信息？")){
        		...
        		$.ajax({
        	        ...
        	        //数据格式是key/value
        	        data:'pId='+pId+'&oid='+oid+'&outype='+outype+'&type=2',
        	        success:function(data){//返回json结果
        	        	if(data!=null){
        	        		if(data==1){
        	            		alert('操作成功');
        	...
        	if(data!=null&&data!=''){
				...
				$(data.list).each(function(index,item){
                  ...
                  //项目中期状态
					...
					if(item.uType==2){//教师
                    	$('#outype').val(0);
                    	if(item.pmiOStatus==21||item.pmiOStatus==22){
                    ...
                    if(item.uType==3){//评审
                    	$('#outype').val(1);
                    	if(item.pmiOStatus==27||item.pmiOStatus==28){
                        	project +='<button type="button" class="am-btn am-btn-default am-btn-xs am-text-secondary" '
                                +' data-toggle="modal" data-target="#newopinion" '
                                +'onclick="putppiId('+item.ppiId+','+item.pbId+')"> 审核</button>'
                                +'<button type="button" class="am-btn am-btn-default am-btn-xs am-text-danger am-hide-sm-only" '
