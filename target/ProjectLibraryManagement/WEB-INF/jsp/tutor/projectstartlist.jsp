...
<head>
    ...
    <title>立项审核-大学生创新创业训练项目库管理系统</title>
		...
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal"
						aria-hidden="true">&times;</button>
					<h4 class="modal-title" id="myModalLabel">项目立项审核</h4>
				</div>
				<div class="modal-body" style="height:400px;overflow:auto;">
					<form role="form">
        ...
        complete:function()
        {
        	...
        	if(confirm("是否确认重置审核信息？")){
        		...
        		$.ajax({
        	        ...
        	        //数据格式是key/value
        	        data:'pId='+pId+'&oid='+oid+'&outype='+outype+'&type=1',
        	        success:function(data){//返回json结果
        	        	if(data!=null){
        	        		if(data==1){
        	            		alert('操作成功');
        ...
        success:function(data){//返回json结果
        	if(data!=null&&data!=''){
				...
				$(data.list).each(function(index,item){
                    ...
                    if(item.uType==2){//教师
                    	$('#outype').val(0);
                    	if(item.psiOStatus==11||item.psiOStatus==12){
                    ...
                    if(item.uType==3){//评审
                    	$('#outype').val(1);
                    	if(item.psiOStatus==17||item.psiOStatus==18){
                        	project +='<button type="button" class="am-btn am-btn-default am-btn-xs am-text-secondary" '
                                +' data-toggle="modal" data-target="#newopinion" '
                                +'onclick="putppiId('+item.ppiId+','+item.pbId+')"> 审核</button>'
                                +'<button type="button" class="am-btn am-btn-default am-btn-xs am-text-danger am-hide-sm-only" '
...
function putppiId(ppid,pid) {
	$('#startppiId').val(ppid);//立项过程信息id
	$('#startpId').val(pid);   //项目id
}
$(function(){
